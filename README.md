# CookBook

A collection of simple and elegant coding problems, that I found were interesting to solve.



### Back Tracking

* [Sudoku Solver](./Back_Tracking/sudoku_solver.py)


### Cryptography

* [Elliptic-Curve](./Cryptography/elliptic_curve.py)
* [Enigma without Plugboard](./Cryptography/enigma_without_plugboard.py)
* [Straddling Checkerboard](./Cryptography/straddling_checkerboard.py)
* [The Devil's Chessboard](./Cryptography/the_devils_chessboard.py)
* [XOR cipher](./Cryptography/xor_cipher.py)


### Divide and Conquer

* [Karatsuba Multiplication](./Divide_and_Conquer/karatsuba_multiplication.py)


### Dyanmic Programming

* [Sequence Alignment](./Dynamic_programming/sequence_alignment.py)
* [Traveling Salesman Problem (Bit mask)](./Dynamic_programming/traveling_salesman_problem_bitmask.py)


### Greedy Algorithm

* [Huffman Code](./Greedy_Algorithm/huffman.py)


### Miscellaneous

* [11 puzzle (Not optimized)](./Miscellaneous/11_puzzle.py)
* [Brainfuck Decoder](./Miscellaneous/brainfuck_decoder.py)
* [Number to Text](./Miscellaneous/num_to_text.py)
* [Roman Addition](./Miscellaneous/roman_addition.py)
